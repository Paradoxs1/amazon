...
class AppTrackingReviewsCreateCommand extends BasicParseCommand
{
    ...
    {
        $this
            ->setName('app:tracking:reviews:create')
            ->setDescription('Creates a queue of urls for reviews parsing.')
            ->addArgument('restartQueue', InputArgument::IS_ARRAY, 'If there is a given argument, the script will get a queue for failed reviews to restart.');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->crawlingQueueType = CrawlingQueue::getTypeReviewsCreate();
        ...
        foreach ($this->trackings as $tracking) {
            ...
            $this->startItemProcessing($this->tracking, 'Initiating create reviews queue');

            $this->processTracking();

            $this->finishItemProcessing('Reviews create queue is finished');
        }
